...
values (1,'WRITE', 'Редактировать спейс', 'space', now(), now()),
       ...
       (5, 'ADD', 'Добавлять новых пользователей в спейс', 'space_user', now(), now()),
       (6, 'DELETE', 'Кикать пользователей из спейса', 'space_user', now(), now()),
       (7, 'READ', 'Смотреть роли и права всех пользователей', 'space_user_actions', now(), now()),
       (8, 'WRITE', 'Изменять роли и права других пользоваталей', 'space_user_actions', now(), now());
